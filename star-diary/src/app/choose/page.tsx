...
export default function Choose() {
  ...
  const [first, second] = match(core.map((e) => e.score!));
  ...
  function pick(id: string) {
    chooseConstellation(id, [first.constellation.id, second.constellation.id], core.map((e) => e.id));
    router.push("/");
  }
    ...
    <main className="relative min-h-screen">
      ...
      <section className="relative mx-auto flex min-h-screen w-full max-w-5xl flex-col justify-center gap-10 px-8 py-16">
        <header className="max-w-2xl">
          <h1 className="font-serif text-2xl">기록이 두 개의 하늘을 가리켰어요</h1>
          <p className="mt-3 leading-relaxed text-muted">
            세 편의 기록에서 <span className="text-starlight">{emotions.join(", ")}</span>이 읽혔고,
            {quotes.length > 0 && <> “{quotes.join("”, “")}” 같은 말이 남았어요.</>}
            {" "}어느 쪽이 지금의 당신에 더 가까운가요? 정답은 없어요 — 고르는 쪽이 당신의 이야기가 됩니다.
          </p>
        </header>

        <div className="grid gap-6 md:grid-cols-2">
          {[first, second].map((cand, i) => {
            ...
            return (
              <article key={c.id} className="flex flex-col rounded-xl border border-starlight/15 bg-sky-low/80 p-8">
                ...
                <p className="mt-2 text-sm text-muted">{i === 0 ? "기록이 가장 가까이 가리킨 곳" : "그다음으로 가까운 곳"}</p>
                ...
                <button onClick={() => pick(c.id)} className={`${btn} mt-auto self-start pt-3`}>
                  이 하늘로 할게요
                </button>
              </article>
            );
